<template>
  <div
    ...
    :style="style"
    :class="classObject"
    @mousedown="handleBodyMouseDown($event)"
    @touchstart.stop.prevent="handleBodyMouseDown($event)"
  >
    <slot></slot>
    <!-- <div
      ...
      :class="['drag-resize-stick-' + stick, resizable ? '' : 'not-resizable']"
...
import { bindEvent, unBindEvent } from "./utils/index.js";
...
onMounted(() => {
  /* 绑定各种事件 */
  // 鼠标移动
  bindEvent("mousemove", handleMouseMove);
  // 鼠标移入
  bindEvent("mouseup", handleMouseUp);
  // 鼠标移出
  bindEvent("mouseleave", handleMouseLeave);
  // 取消选中
  bindEvent("deselect", handleDeselect);

  // 移动端-触摸移动
  bindEvent("touchmove", handleTouchMove);
  // 移动端-结束触摸、取消触摸
  bindEvent("touchend touchcancel", handleTouchEnd);
  // 移动端-触摸开始
  bindEvent("touchstart", handleTouchStart);
});

onUnmounted(() => {
  /* 解绑各种事件 */
  unBindEvent("mousemove", handleMouseMove);
  unBindEvent("mouseup", handleMouseUp);
  unBindEvent("mouseleave", handleMouseLeave);
  unBindEvent("deselect", handleDeselect);

  unBindEvent("touchmove", handleTouchMove);
  unBindEvent("touchend touchcancel", handleTouchEnd);
  unBindEvent("touchstart", handleTouchStart);
});
</script>
...
.drag-resize.selectable.inactive:hover:before {
  content: "";
  width: 100%;
  height: 100%;
  position: absolute;
